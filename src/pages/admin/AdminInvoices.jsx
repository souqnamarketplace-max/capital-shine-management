...
export default function AdminInvoices() {
  ...
  const sendEmail = async (inv) => {
    ...
    const attachmentHtml = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Invoice ${inv.invoiceNumber}</title><style>body{font-family:Arial,sans-serif;color:#1a1a2e;padding:40px;max-width:700px;margin:0 auto}h2{color:#0d2b5e}table{width:100%;border-collapse:collapse;margin:20px 0}th{background:#0d2b5e;color:white;padding:10px 12px;text-align:left;font-size:12px}td{padding:8px 12px;border-bottom:1px solid #f0f0f0}.totals{text-align:right;margin-top:10px}.tot-row{display:flex;justify-content:flex-end;gap:40px;font-size:13px;padding:4px 0}.grand{font-weight:bold;font-size:16px;border-top:2px solid #0d2b5e;padding-top:8px;margin-top:8px}</style></head><body>
      ...
      <div class="totals"><div class="tot-row"><span>Subtotal</span><span>$${Number(inv.subtotal).toFixed(2)}</span></div><div class="tot-row"><span>Tax (${inv.taxRate}%)</span><span>$${Number(inv.taxAmount).toFixed(2)}</span></div><div class="tot-row grand"><span>Total</span><span>$${Number(inv.total).toFixed(2)}</span></div></div>
      ${inv.notes ? `<div style="margin-top:24px;padding:12px;background:#f8f9ff;border-radius:8px;font-size:13px;color:#555"><strong>Notes:</strong> ${inv.notes}</div>` : ''}
    </body></html>`;
    await base44.functions.invoke('sendClientEmail', { to: inv.clientEmail, subject: `Invoice ${inv.invoiceNumber} from Capital Shine`, body, attachmentHtml, attachmentFilename: `Invoice-${inv.invoiceNumber}.html` });
